...
from keras.layers import Dense, LSTM, Embedding, Dropout, Activation, Bidirectional, Flatten
...
model.add(Bidirectional(LSTM(num_lstm, dropout=rate_drop_lstm, recurrent_dropout=rate_drop_lstm, return_sequences=True)))
model.add(SeqSelfAttention(units=10, attention_type='multiplicative',attention_activation='sigmoid'))
model.add(Bidirectional(LSTM(10, dropout=rate_drop_lstm, recurrent_dropout=rate_drop_lstm, return_sequences=False)))
...
STAMP = 'bi_lstm_w2v300_att_masked'

from keras.callbacks import EarlyStopping, ModelCheckpoint

early_stopping = EarlyStopping(monitor='val_loss', patience=3)
best_model_path = path + 'bi_lstm_att/' + STAMP + '.h5'
...
with open(path+'bi_lstm_att/history_'+STAMP+'_1.p', 'wb') as f:
